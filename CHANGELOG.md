...
vsync off, 3 rounds x 300 samples per station. Two arms, because a shipping
drip would not touch every wall family and dividing the wide figure by a
material ratio is exactly the kind of arithmetic 0.115.0 retracted:
    ...
    us per added draw     --   2.37-7.02 (med 4.74)  2.14-7.30 (med 4.24)

`drip_few` is the narrow set a drip actually wants -- brick, siding, stucco,
corrugated, shingle, the street-facing families. `drip` is the wide set (15
name patterns, 250 materials) and is the upper bound, not a proposal.

MATERIAL COUNT DOES NOT PREDICT COST, and measuring the narrow arm rather than
scaling the wide one is the whole reason this entry is trustworthy:

    material ratio   wide / few = 13.16x
    worst-station ms ratio      =  4.65x  (8.60 / 1.85)

Had 0.116.0 divided the 250-material figure by the material ratio it would have
predicted +0.65 ms for 19 families and been wrong by nearly 3x in the
unaffordable direction. Cost tracks SCREEN COVERAGE, not the length of the
material list: brick and siding are large surfaces, so 19 of them fill far more
of a street frame than their 7.6% share of the list suggests. This is the same
error as the retraction wearing different clothes -- a cheap observable (a
count) standing in for an expensive truth (fill) -- and the only thing that
caught it was running the arm.

BOTH CONTROLS FIRED, which is the condition 0.115.0 imposed:

    draw calls rose         at 6 of 6 stations
    frame luminance changed at 5 of 6 stations

The second is the one that matters and the one the withdrawn measurement did
not have. Draw calls rising proves a submission; the frame changing proves a
fragment ran.

WHAT THE FRAGMENT IS. Pixelcoat 0.50.0's drop atlas (`core/droplets.py`): RG
is the drop normal's XY, B a per-drop random giving each drop its own clock, A
the small drops surface tension holds still. `fract(b - TIME * speed) * b`
decays a drop from its own random value to zero and snaps back; `smoothstep`
on the world up-vector keeps rain off a soffit; `NORMAL_MAP` is written and
roughness cut where there is water. No screen read and no depth read -- that
is Reference B, whose availability on GL Compatibility is still unmeasured
(RAIN_WETNESS.md item 1), and pricing it here would price two things at once.
The atlas is GENERATED by Pixelcoat into the staged arm rather than checked in
beside the probe, because a drip measured without its texture fetch is not a
drip.

WHAT IS NOT MEASURED. One package, one theme, six stations -- closest to the
performance contract's "typical traversal" and "worst sightline", not its nine
scenarios. `ground_near` read 1.93 ms with the pass against 2.04 ms without in
a single round, which is noise at a near-empty station and is why the figures
above are the three-round medians.

TOOLING. `wet_ab_run.py --arms` selects any subset of the six arms (`dry` is
always re-added, since every figure is a difference against it) and
`--pixelcoat` points at the repo the atlas comes from. Four places in the
report had the four original arm names written into them, including the
did-it-draw control -- so a run without a `wet` arm could not check itself --
and now derive from the arms that ran. The runner REFUSES any non-dry arm that
attached to zero materials, not just `wet`; an arm whose name test found
nothing is the dry arm measured twice, and its figures would read as free
rather than as never run.
